'use client';
import shortenUrlOperaton, { ErrorResponse, ShortenUrlResponse } from '../../app/api/UrlShortnerApi';
...
export default function ShortenUrlForm() {
    ...
    const [shortenedUrlState, setShortenedUrlState] = useState({errorResponse: null, shortenedUrl: ''})
    const [openModalUrlState, setOpenModalUrlState] = useState(false)
    const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        const input = e.target.value
        const result = input.match(/(http(s)?:\/\/.)?(www\.)?[-a-zA-Z0-9@:%._\+~#=]{2,256}\.[a-z]{2,6}\b([-a-zA-Z0-9@:%_\+.~#?&=]*)/g) != null
                ...
                <Grid container sx={{ justifyContent: 'center', alignItems: 'center' }}>
                    <Box padding={2}>
                        ...
                        <Typography id="modal-modal-description">
                            URL:
                            <Link sx={{ padding: 0.5 }} href={shortenedUrlState.shortenedUrl} underline="hover">
        ...
        <div>
            <FormGroup>
                <FormControl>
                    ...
                    <Input type="url" value={urlInputState} onChange={handleInputChange} placeholder="https://www.example.com" id="url-input" required />
                    <Button onClick={handleSubmit} loading={buttonIsLoadingState} disabled={!buttonIsActiveState}>Shorten!</Button>
                    {successDialog()}
